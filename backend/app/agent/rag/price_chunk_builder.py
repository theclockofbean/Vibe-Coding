"""Build Price KB chunks from price_questions.xlsx."""
...
def build_price_kb_chunks_from_excel(
    *,
    price_file: Path,
    collection_name: str = DEFAULT_COLLECTION_NAME,
    sheet_name: str = DEFAULT_SHEET_NAME,
) -> list[PriceKBChunk]:
    """Build Price KB chunks from Excel file."""
...
def build_price_kb_chunk(
    *,
    record: dict[str, Any],
    source_row_index: int,
    collection_name: str = DEFAULT_COLLECTION_NAME,
) -> PriceKBChunk:
    """Build one Price KB chunk."""
    ...
    if primary_intent != "price":
        raise ValueError(
            f"row {source_row_index}: primary_intent must be price, got {primary_intent}"
        )

    title_question = question_normalized or question_raw
    doc_title = f"{qa_id}｜价格问答｜{intent_subtype}｜{title_question}"
    ...
    return PriceKBChunk(
        chunk_id=f"price_qa_{qa_id.lower()}",
        doc_id=f"price_qa_{qa_id.lower()}",
        doc_title=doc_title,
        content=content,
        summary=f"{qa_id}｜价格问答｜{intent_subtype}｜{title_question}",
        source_row_index=source_row_index,
        collection_name=collection_name,
        module="price",
        source_type="qa_pair",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"备注：{review_notes}",
            "边界规则：价格类问题不得直接报价、不得承诺最低价、不得承诺折扣。",
            "边界规则：金额和优惠必须经过正式核算或人工确认。",
        ]
    )
