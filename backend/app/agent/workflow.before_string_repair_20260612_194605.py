...
class AgentWorkflowNodes:
    ...
    def handler_node(self, state: AgentState) -> AgentState:
        ...
        if not user_text:
            ...
            next_state["answer_text"] = "鐢ㄦ埛闂涓虹┖锛屼笉鑳借嚜鍔ㄥ鐞嗭紝璇疯浆浜哄伐纭銆?
        ...
        try:
            ...
            next_state["handler_status"] = "handoff"
            next_state["answer_text"] = "绯荤粺澶勭悊褰撳墠闂鏃跺彂鐢熷紓甯革紝璇疯浆浜哄伐纭銆?
            next_state["handoff_required"] = True
            next_state["human_handoff"] = True
            _append_warning(
                next_state,
...
def _build_llm_business_rules() -> list[str]:
    ...
    return [
        "LLM 杈撳嚭涓嶆槸浜嬪疄鏉ユ簮銆?,
        "LLM 涓嶅緱鐢熸垚浠锋牸銆佸簱瀛樸€佺墿娴併€佽川閲忋€佸敭鍚庢壙璇恒€?,
        "鏈€缁堢粨璁哄繀椤讳互缁撴瀯鍖栨暟鎹€佷笟鍔¤鍒欐垨浜哄伐纭涓哄噯銆?,
        "璇佹嵁涓嶈冻鏃跺簲鎷掔瓟鎴栬浆浜哄伐銆?,
    ]
