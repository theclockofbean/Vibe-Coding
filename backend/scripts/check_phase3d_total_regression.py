"""Run Phase 3-D total regression checks.

This script runs Phase 3-C total regression plus LangGraph workflow checks.
...
promise compensation, or create business commitments.
"""
...
REGRESSION_SCRIPTS: Final[tuple[RegressionScript, ...]] = (
    RegressionScript(
        ...
        relative_path="check_phase3c_total_regression.py",
    ),
    RegressionScript(
        name="langgraph_installation",
        relative_path="check_langgraph_installation.py",
    ),
    RegressionScript(
        name="langgraph_workflow_skeleton",
        relative_path="check_langgraph_workflow_skeleton.py",
    ),
    RegressionScript(
        name="agent_workflow_vs_unified_api",
        relative_path="check_agent_workflow_vs_unified_api.py",
    ),
)
...
def print_summary(results: list[RegressionResult]) -> None:
    ...
    print("=" * 100)
    print("phase3-d total regression summary")
...
def main() -> int:
    """Run all Phase 3-D total regression scripts."""

    print("=" * 100)
    print("starting phase3-d total regression")
    print(f"backend_root: {BACKEND_ROOT}")
    print(f"python: {sys.executable}")

    if not validate_scripts_exist():
        print("phase3-d total regression failed")
        return 1
    ...
    print("phase3-d total regression passed")
    return 0
