"""Check LogisticsTextQAService.

This script verifies the full local logistics text QA chain:
parser -> handler -> renderer -> response payload.

It does not call an LLM, promise delivery time, calculate shipping fees,
...
class LogisticsTextQACheckCase:
    """One logistics text QA service check case."""
...
def build_cases() -> list[LogisticsTextQACheckCase]:
    """Return deterministic text QA service check cases."""

    return [
        LogisticsTextQACheckCase(
            text="SKU001 几天发货",
...
def main() -> int:
    """Run logistics text QA service checks."""
    ...
    if not all(results):
        print("logistics text QA service check failed")
        return 1

    print("logistics text QA service check passed")
    return 0
