"""Run Phase 3-B total regression checks.

This script runs stable Phase 1, Phase 2, Phase 3-A, and Phase 3-B checks.
...
REGRESSION_SCRIPTS: Final[tuple[RegressionScript, ...]] = (
    ...
    RegressionScript(
        ...
        relative_path="check_conversation_schema.py",
    ),
    RegressionScript(
        name="conversation_repository",
        relative_path="check_conversation_repository.py",
    ),
    RegressionScript(
        name="conversation_service",
        relative_path="check_conversation_service.py",
    ),
    RegressionScript(
        name="conversation_api",
        relative_path="check_conversation_api.py",
    ),
    RegressionScript(
        name="unified_agent_conversation_integration",
        relative_path="check_unified_agent_conversation_integration.py",
    ),
)
...
def print_summary(results: list[RegressionResult]) -> None:
    ...
    print("=" * 100)
    print("phase3-b total regression summary")
...
def main() -> int:
    """Run all Phase 3-B total regression scripts."""

    print("=" * 100)
    print("starting phase3-b total regression")
    print(f"backend_root: {BACKEND_ROOT}")
    print(f"python: {sys.executable}")

    if not validate_scripts_exist():
        print("phase3-b total regression failed")
        return 1
    ...
    print("phase3-b total regression passed")
    return 0
