# ruff: noqa: E402,I001
"""Check workflow integration with real Logistics KB retrieval."""
...
TEST_QUERY: Final[str] = "SKU001今天拍什么时候发货？"


def check_workflow_logistics_kb_retrieval_integration() -> bool:
    """Check workflow uses real Logistics KB retrieval for logistics query."""

    print("=" * 80)
    print("checking workflow real Logistics KB retrieval integration")
        ...
        initial_state = cast(
            AgentState,
            {
                ...
                "metadata": {
                    ...
                    "test_case": "workflow_logistics_kb_retrieval_integration",
                },
            },
        )
    ...
    if metadata.get("real_logistics_kb_retriever_used") is not True:
        errors.append("real_logistics_kb_retriever_used must be true")

    if metadata.get("retrieval_source") != "real_logistics_kb":
        errors.append("retrieval_source must be real_logistics_kb")

    if metadata.get("retrieval_selected_module") != "logistics":
        errors.append("retrieval_selected_module must be logistics")

    if metadata.get("retrieval_collection_name") != "logistics_kb_v1":
        errors.append("retrieval_collection_name must be logistics_kb_v1")
    ...
    if retrieved_chunks:
        ...
        if top_chunk.get("module") != "logistics":
            errors.append("top retrieved chunk module must be logistics")
    ...
    safe_result = {
        ...
        "metadata": {
            ...
            for key in (
                "real_logistics_kb_retriever_enabled",
    ...
    if errors:
        print("workflow real Logistics KB retrieval integration check failed")
        return False

    print("workflow real Logistics KB retrieval integration check passed")
    return True
...
def main() -> int:
    ...
    try:
        passed = check_workflow_logistics_kb_retrieval_integration()
    except Exception as exc:
        print(
            "workflow real Logistics KB retrieval integration check crashed: "
