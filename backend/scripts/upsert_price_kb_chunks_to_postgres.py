# ruff: noqa: E402,I001
"""Upsert Price KB chunks to PostgreSQL knowledge_chunks."""
...
def main() -> int:
    """Upsert Price KB chunks."""

    print("=" * 80)
    print("upserting Price KB chunks to PostgreSQL")
    ...
    with session_factory() as session:
        ...
        for chunk in chunks:
            ...
            upsert_chunk(session=session, row=row, table_columns=table_columns)
            upserted_count += 1

        session.commit()

        db_count = count_price_chunks(session=session)
    ...
    if errors:
        print("Price KB PostgreSQL upsert failed")
        return 1

    print("Price KB PostgreSQL upsert passed")
    return 0
...
def count_price_chunks(
    *,
    session: Any,
) -> int:
    """Count active Price KB chunks."""
