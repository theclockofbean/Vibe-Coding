"""Smoke test for local controlled price API.
...
class PriceApiCheckCase:
    """One price API smoke test case."""

    text: str
    expected_http_status: int
    expected_parse_status: str | None
    expected_handler_status: str | None
    expected_is_price_intent: bool | None
    expected_handoff_required: bool | None
    expected_answer_fragment: str | None


def build_cases() -> list[PriceApiCheckCase]:
...
def run_case(
    *,
    client: httpx.Client,
    base_url: str,
    case: PriceApiCheckCase,
) -> bool:
    """Run one price API check case."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Smoke test local controlled price API.",
    )
    parser.add_argument(
        "--base-url",
...
def main() -> int:
    ...
    if not all(results):
        print("price API smoke test failed")
        return 1

    print("price API smoke test passed")
    return 0
