# ruff: noqa: E402,I001
"""Check Answer Strategy risk handoff gate."""
...
RISK_CASES: Final[tuple[dict[str, object], ...]] = (
    {
...
SAFETY_CASES: Final[tuple[dict[str, object], ...]] = (
    {
        "query": "SKU001给我最低价",
        "selected_module": "price",
        "candidate_modules": ["price"],
...
def main() -> int:
    """Run answer strategy risk gate check."""

    print("=" * 80)
    print("checking Phase 3-I-I answer strategy risk gate")
    ...
    for case in RISK_CASES:
        ...
        if result["strategy_mode"] != "handoff_required":
            errors.append(
                f"risk case expected handoff_required, got {result}"
            )

        if result["handoff_required"] is not True:
            errors.append(f"risk case did not require handoff: {result}")

        if result["safety_blocked"] is not False:
            errors.append(f"risk handoff case should not be safety blocked: {result}")

    for case in SAFETY_CASES:
        result = evaluate_case(case=case, config=config)
        results.append(result)

        if result["strategy_mode"] != "safety_blocked":
            errors.append(
                f"safety case expected safety_blocked, got {result}"
            )

        if result["safety_blocked"] is not True:
            errors.append(f"safety case was not blocked: {result}")
    ...
    for case in NORMAL_CASES:
        ...
        if result["strategy_mode"] != "single_primary":
            errors.append(
                f"normal case expected single_primary, got {result}"
            )
    ...
    if errors:
        print("Phase 3-I-I answer strategy risk gate check failed")
        return 1

    print("Phase 3-I-I answer strategy risk gate check passed")
    return 0
...
def evaluate_case(
    *,
    ...
    """Evaluate one answer strategy case."""

    query = str(case["query"])
    selected_module = str(case["selected_module"])
